...
/**
 * Same JSON as GET /api/user/settings, authenticated with
...
export async function GET(req: Request) {
  try {
    ...
    console.error("[mobile/user/settings GET]", e);
...
export async function PATCH(req: Request) {
  try {
    ...
    const db = await (async () => {
      ...
      try {
        ...
        if (isPermissionDenied(e)) {
          console.warn("[mobile/user/settings] PATCH: service_role denied; using session client for DB");
          await ensureSettingsRow(auth, user.id);
          await ensureProfileRow(auth, user);
          return auth;
        }
    ...
    console.error("[mobile/user/settings PATCH]", e);
