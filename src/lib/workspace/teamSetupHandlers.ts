...
const TEAM_SETUP_ROUTE = "/api/workspace/team-setup";

export async function teamSetupGetHandler(supabase: SupabaseClient, user: User): Promise<NextResponse> {
...
export async function teamSetupPatchHandler(supabase: SupabaseClient, user: User, req: Request): Promise<NextResponse> {
  let viewerId: string | null = null;
  try {
    viewerId = user.id;
